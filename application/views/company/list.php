<!DOCTYPE html>
        <span width="100" style="background-color: #e0e0e0; display: block;">
            &nbsp;<font size="5" style="font-weight: bold; color: #737373">Company Management</font>
        </span>
        <br/>
        <?if($this->session->userdata("role") == 1 || $this->session->userdata("role") == 2):?>
        <button class="btn btn-success" onclick="add_com()"><i class="glyphicon glyphicon-plus"></i> Add Company</button>
        <!--<button class="btn btn-success" onclick="upload_com()"><i class="glyphicon glyphicon-plus"></i> Upload Company</button>-->
        <?endif;?>
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('index.php/Company/ajax_list')?>",
            "type": "POST",
            data : dataJson
        },
...
function upload_com()
{
    window.location = _base_url + 'index.php/Company/new_upload_com/';
}
...
//    $('.modal-title').text('Edit Company'); // Set Title to Bootstrap modal title
    ...
    $.ajax({
            ...
            'success': function (data) {
                ...
                $.ajax({
                    url : "<?php echo site_url('index.php/Company/ajax_edit/')?>/" + id.replace('/', 'slash'),
                    type: "GET",
                    dataType: "JSON",
                    success: function(data)
                    {
            
                        $('[name="comId"]').val(data.company_id);
                        $('[name="comName"]').val(data.company_name);
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('index.php/Company/ajax_add')?>";
    } else {
        url = "<?php echo site_url('index.php/Company/ajax_update')?>";
    }
    ...
    if(confirm('Are you sure delete this emp: ' + emp + ' ?'))
    {
        $.ajax({
            ...
            'success': function (data) {
                ...
                $.ajax({
                    url : "<?php echo site_url('index.php/Company/ajax_delete')?>/"+ id.replace('/', 'slash'),
                    type: "POST",
                    dataType: "JSON",
                    data: dataJson,
                    success: function(data)
...
function view_division(com_id) {
  window.location = _base_url + 'index.php/Division/lists/' + com_id;
}
</script>
                ...
                <form action="#" id="form" class="form-horizontal">
                    ...
                    <div class="form-body">
                        <div class="form-group">
                            <label class="control-label col-md-3">Company ID</label>
                            <div class="col-md-9">
                                <input name="comId" placeholder="Company ID" class="form-control" type="text" readonly="true">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <input name="comName" placeholder="Company Name" class="form-control" type="text">
                                <span class="help-block"></span>
                            </div>
                        </div>
                        <div class="form-group">
