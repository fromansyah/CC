        ...
        <span width="100" style="background-color: #e0e0e0; display: block;">
            &nbsp;<font size="5" style="font-weight: bold; color: #737373">Branch Management : <?echo $company_name?></font>
        </span>
        <br/>
        <button class="btn btn-warning" onclick="back_to_company()"><i class="glyphicon glyphicon-backward"></i> Back</button>
        <button class="btn btn-success" onclick="add_branch()"><i class="glyphicon glyphicon-plus"></i> Add</button>
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('index.php/Branch/ajax_list/'.$company_id)?>",
            "type": "POST",
            data : dataJson
        },
...
function edit_branch(company_id)
{
    save_method = 'update';
    $('#form')[0].reset(); // reset form on modals
    $('.form-group').removeClass('has-error'); // clear error class
    ...
    $.ajax({
            ...
            'success': function (data) {
                ...
                $.ajax({
                    url : "<?php echo site_url('index.php/Branch/ajax_edit/')?>" + company_id,
                    type: "GET",
                    dataType: "JSON",
                    success: function(data)
                    {
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('index.php/Branch/ajax_add')?>";
    } else {
        url = "<?php echo site_url('index.php/Branch/ajax_update')?>";
    }
...
function delete_branch(branch_id, branch_name)
{
    if(confirm('Are you sure delete this data: ' + branch_name + ' ?'))
    {
        $.ajax({
            'async': true,
            'type': "GET",
            ...
            'success': function (data) {
                ...
                $.ajax({
                    url : "<?php echo site_url('index.php/Branch/ajax_delete')?>/"+ branch_id,
                    type: "POST",
                    dataType: "JSON",
                    data: dataJson,
                    success: function(data)
...
function view_branch(company_id) {
  window.location = _base_url + 'index.php/Branch/lists/' + company_id;
}
</script>
