        ...
        <span width="100" style="background-color: #e0e0e0; display: block;">
            &nbsp;<font size="5" style="font-weight: bold; color: #737373">CAT Sheet Management - <?=$product_name?></font>
        </span>
        <br/>
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('Cat_sheet/ajax_list/')?>"+ _product_id,
            "type": "POST",
            "data": dataJson
        },
...
function edit_cat_sheet(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('Cat_sheet/ajax_edit/')?>/" + id,
        type: "GET",
        dataType: "JSON",
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('Cat_sheet/ajax_add')?>";
    } else {
        url = "<?php echo site_url('Cat_sheet/ajax_update')?>";
    }
...
function delete_cat_sheet(id, cat_sheet)
{
    if(confirm('Are you sure delete this data: ' + cat_sheet + ' ?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('Cat_sheet/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            data: dataJson,
            success: function(data)
...
function cat_sheet($id){
        //alert($lang);
        var win = window.open(_base_url+'Cat_sheet/view_cat_sheet/'+ $id, '_blank');
        win.focus();
    }
</script>
...
<div class="modal fade" id="modal_form" cat_sheet="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="<?=base_url()?>Cat_sheet/save_cat_sheet" method="post" id="form" class="form-horizontal" enctype="multipart/form-data">
